...
with_theme(My_theme, palette=(color=ColorSchemes.Set1_3, marker=[:circle])) do
    ...
    CMRipp = C_MR.(λ, λ .^ (-0.41), ρs, VL, MRfit[1], MRfit[2], fip, τ, n, βMR + βMRerror)
    VSHpartp, VSHperptp, VSpartp, VSperptp = lw_mode_velocity(λ, ρs, CMRipp)

    lines!(ax2, λ, real(VSHpart) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1])
    band!(ax2, λ, real(VSHpart) ./ sqrt(sum(MRfit) / ρs), real(VSHpartp) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1], alpha = 0.5)
    lines!(ax2, λ, real(VSpart) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1])
    band!(ax2, λ, real(VSpart) ./ sqrt(sum(MRfit) / ρs), real(VSpartp) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1], alpha = 0.5)
    lines!(ax3, λ, real(VSHperpt) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1])
    band!(ax3, λ, real(VSHperpt) ./ sqrt(sum(MRfit) / ρs), real(VSHperptp) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1], alpha = 0.5)
    lines!(ax3, λ, real(VSperpt) ./ sqrt(sum(MRfit) / ρs), color=ColorSchemes.Set1_3[1])
    band!(ax3, λ, real(VSperptp) ./ sqrt(sum(MRfit) / ρs), real(VSperpt) ./ sqrt(sum(MRfit) / ρs),color=ColorSchemes.Set1_3[1], alpha = 0.5)

    CGTip = C_GT.(λ, λ .^ (-0.41), ρs, VL, GTfit[1], GTfit[2], fip, τ, n, βGT)
    VSHpart, VSHperpt, VSpart, VSperpt = lw_mode_velocity(λ, ρs, CGTip)
    CGTipp = C_GT.(λ, λ .^ (-0.41), ρs, VL, GTfit[1], GTfit[2], fip, τ, n, βGT .+ βGTerror)
    VSHpartp, VSHperptp, VSpartp, VSperptp = lw_mode_velocity(λ, ρs, CGTipp)

    lines!(ax2, λ, real(VSHpart) ./ sqrt(sum(GTfit) / ρs), color=ColorSchemes.Set1_3[2])
    band!(ax2, λ, real(VSHpart) ./ sqrt(sum(GTfit) / ρs), real(VSHpartp) ./ sqrt(sum(GTfit) / ρs),  color=ColorSchemes.Set1_3[2], alpha = 0.5)
    lines!(ax2, λ, real(VSpart) ./ sqrt(sum(GTfit) / ρs), color=ColorSchemes.Set1_3[2])
    band!(ax2, λ, real(VSpart) ./ sqrt(sum(GTfit) / ρs), real(VSpartp) ./ sqrt(sum(GTfit) / ρs),  color=ColorSchemes.Set1_3[2], alpha = 0.5)
    lines!(ax3, λ, real(VSHperpt) ./ sqrt(sum(GTfit) / ρs), color=ColorSchemes.Set1_3[2])
    band!(ax3, λ, real(VSHperpt) ./ sqrt(sum(GTfit) / ρs), real(VSHperptp) ./ sqrt(sum(GTfit) / ρs),  color=ColorSchemes.Set1_3[2], alpha = 0.5)
    lines!(ax3, λ, real(VSperpt) ./ sqrt(sum(GTfit) / ρs), color=ColorSchemes.Set1_3[2])
    band!(ax3, λ, real(VSperpt) ./ sqrt(sum(GTfit) / ρs), real(VSperptp) ./ sqrt(sum(GTfit) / ρs),  color=ColorSchemes.Set1_3[2], alpha = 0.5)

    CCip = C_C.(λ, λ .^ (-0.41), ρs, VL, Cfit[1], Cfit[2], fip, τ, n, βC)
    VSHpart, VSHperpt, VSpart, VSperpt = lw_mode_velocity(λ, ρs, CCip)
    CCipp = C_C.(λ, λ .^ (-0.41), ρs, VL, Cfit[1], Cfit[2], fip, τ, n, βC + βCerror)
    VSHpartp, VSHperptp, VSpartp, VSperptp = lw_mode_velocity(λ, ρs, CCipp)

    lines!(ax2, λ, real(VSHpart) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3])
    band!(ax2, λ, real(VSHpart) ./ sqrt(sum(Cfit) / ρs), real(VSHpartp) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3], alpha = 0.5)
    lines!(ax2, λ, real(VSpart) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3])
    band!(ax2, λ, real(VSpart) ./ sqrt(sum(Cfit) / ρs), real(VSpartp) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3], alpha = 0.5)
    lines!(ax3, λ, real(VSHperpt) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3])
    band!(ax3, λ, real(VSHperpt) ./ sqrt(sum(Cfit) / ρs), real(VSHperptp) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3], alpha = 0.5)
    lines!(ax3, λ, real(VSperpt) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3])
    band!(ax3, λ, real(VSperpt) ./ sqrt(sum(Cfit) / ρs), real(VSperptp) ./ sqrt(sum(Cfit) / ρs), color=ColorSchemes.Set1_3[3], alpha = 0.5)
