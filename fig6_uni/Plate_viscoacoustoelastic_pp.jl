...
@unpack GGfit, βGG, DCGTfit, βDCGT = data
...
@time for λ1 in 1:0.1:2.5
    ...
    for i in axes(ω, 1)
        ...
        # μs = sum(Cfit)
        # C = C_MRSH(λ3, λ1, ρs, VL, MRSHfit[1], MRSHfit[2], MRSHfit[3], MRSHfit[4], ω[i] / (2 * pi), τ, n, βMRSH[1])
        # μs = sum(MRSHfit[1:3])
        # C = C_GTSH(λ3, λ1, ρs, VL, GTSHfit[1], GTSHfit[2], GTSHfit[3], GTSHfit[4], ω[i] / (2 * pi), τ, n, βGTSH[1])
        # μs = sum(GTSHfit[1:3])
        # C = C_CSH(λ3, λ1, ρs, VL, CSHfit[1], CSHfit[2], CSHfit[3], CSHfit[4], ω[i] / (2 * pi), τ, n, βCSH[1])
        # μs = sum(CSHfit[1:3])
        # C = C_GG(λ3, λ1, ρs, VL, GGfit[1], GGfit[2], GGfit[3], ω[i] / (2 * pi), τ, n, βGG[1])
        # μs = sum(GGfit[1:2])
        C = C_DCGT(λ3, λ1, ρs, VL, DCGTfit[1], DCGTfit[2], DCGTfit[3], ω[i] / (2 * pi), τ, n, βDCGT[1])
        μs = sum(DCGTfit[1:2])
        C = C ./ μs
    ...
    save(joinpath(@__DIR__, string("DCGT_perp_", h, "m_", λ1, ".jld2")), "k", ksave, "f", fsave)
