...
simu = filter(x -> occursin("GG", x), readdir(joinpath(@__DIR__, "fig6_uni")))
VSGG = Vector{Float64}()
for name in simu
    data = load(joinpath(@__DIR__, "fig6_uni", name))
    @unpack k, f = data
    _, idx = findmin(abs.(f .- fop))
    append!(VSGG, real(2 * pi * f[idx] / k[idx]))
end
simu = filter(x -> occursin("DCGT", x), readdir(joinpath(@__DIR__, "fig6_uni")))
...
# %%
λ = 1:0.02:3.5
with_theme(My_theme, palette=(color=ColorSchemes.Set1_5, marker=[:circle])) do
    ...
    lines!(ax1, 1 ./ λ, GTSH(λ, GTSHfit), label=L"Gent-Thomas with $I_1^N$ term", color=:black, linestyle=:dash)
    lines!(ax1, 1 ./ λ, GG(λ, GGfit) / 1e3, label="Gent-Gent")
    lines!(ax1, 1 ./ λ, DCGT(λ, DCGTfit) / 1e3, label="Dobrynin-Carrillo-Gent", linestyle=:dash)
    ...
    # add predictions
    CGTSHip = C_GTSH.(λ, λ .^ (-0.5), ρs, VL, GTSHfit[1], GTSHfit[2], GTSHfit[3], GTSHfit[4], fip, τ, n, βGTSH)
    VSHpart, VSHperpt, VSpart, VSperpt, VApart, VAperpt = lw_mode_velocity(λ, ρs, CGTSHip)
    lines!(ax2, λ, real(VSHpart) ./ sqrt(sum(GTSHfit[1:3]) / ρs), color=:black, linestyle=:dash)
    ...
    CGGip = C_GG.(λ, λ .^ (-0.5), ρs, VL, GGfit[1], GGfit[2], GGfit[3], fip, τ, n, βGG)
    ...
    CDCGTip = C_DCGT.(λ, λ .^ (-0.5), ρs, VL, DCGTfit[1], DCGTfit[2], DCGTfit[3], fip, τ, n, βDCGT)
    ...
    CGTSHop = C_GTSH.(λ, λ .^ (-0.5), ρs, VL, GTSHfit[1], GTSHfit[2], GTSHfit[3], GTSHfit[4], fop, τ, n, βGTSH)
    VSHpart, VSHperpt, VSpart, VSperpt, VApart, VAperpt = lw_mode_velocity(λ, ρs, CGTSHop)
    lines!(ax22, λ, real(VApart) ./ sqrt(sum(GTSHfit[1:3]) / ρs), color=:black, linestyle=:dash)
    # lines!(ax23, λ, real(VAperpt) ./ sqrt(sum(GTSHfit[1:3]) / ρs), color=ColorSchemes.Set1_3[1], linestyle=:dash)
    lines!(ax23, λS, VSGTSH1 ./ sqrt(sum(GTSHfit[1:3]) / ρs), color=:black, linestyle=:dash)
    CGGop = C_GG.(λ, λ .^ (-0.5), ρs, VL, GGfit[1], GGfit[2], GGfit[3], fop, τ, n, βGG)
    _, _, _, _, VApart, VAperpt = lw_mode_velocity(λ, ρs, CGGop)
    lines!(ax22, λ, real(VApart) ./ sqrt(sum(GGfit[1:2]) / ρs), color=ColorSchemes.Set1_3[1])
    CDCGTop = C_DCGT.(λ, λ .^ (-0.5), ρs, VL, DCGTfit[1], DCGTfit[2], DCGTfit[3], fip, τ, n, βDCGT)
    _, _, _, _, VApart, VAperpt = lw_mode_velocity(λ, ρs, CDCGTop)
    lines!(ax22, λ, real(VApart) ./ sqrt(sum(DCGTfit[1:2]) / ρs), color=ColorSchemes.Set1_5[2], linestyle=:dash)
    # lines!(ax23, λ, real(VAperpt) ./ sqrt(sum(DCMRfit2.param[1:2]) / ρs), color = ColorSchemes.Set1_5[4], linestyle = :dash)

    lines!(ax23, λS, VSGG ./ sqrt(sum(GGfit[1:2]) / ρs), color=ColorSchemes.Set1_3[1])
    # lines!(ax23, λS, VSGMR ./ sqrt(sum(GGfit[1:2]) / ρs), color=ColorSchemes.Set1_3[1])
    # lines!(ax23, λS, VSGC ./ sqrt(sum(GCfit.param[1:2]) / ρs), color=ColorSchemes.Set1_3[3])
    lines!(ax23, λS, VSDCGT ./ sqrt(sum(DCGTfit[1:2]) / ρs), color=ColorSchemes.Set1_5[2], linestyle=:dash)
    ...
    display(fig)
    save(joinpath(@__DIR__, "figure6_uniaxial.pdf"), fig; pt_per_unit=1.0)
