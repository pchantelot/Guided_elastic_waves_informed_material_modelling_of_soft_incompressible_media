...
with_theme(My_theme, palette=(color=ColorSchemes.Set1_5, marker=[:circle])) do
    ...
    CGGop = C_GG.(λ, λ .^ (-0.39), ρs, VL, GGfit[1], GGfit[2], GGfit[3], fop, τ, n, βGG)
    ...
    CDCGTop = C_DCGT.(λ, λ .^ (-0.39), ρs, VL, DCGTfit[1], DCGTfit[2], DCGTfit[3], fip, τ, n, βDCGT)
    ...
    display(fig)
    save(joinpath(@__DIR__, "figure6.pdf"), fig; pt_per_unit=1.0)
