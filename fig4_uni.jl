...
with_theme(My_theme, palette=(color=ColorSchemes.Set1_3, marker=[:circle])) do
    ...
    CGTip = C_GT.(λ, λ .^ (-0.5), ρs, VL, GTfit[1], GTfit[2], fip, τ, n, βGT)
    ...
    CCip = C_C.(λ, λ .^ (-0.5), ρs, VL, Cfit[1], Cfit[2], fip, τ, n, βC)
    ...
    CGTop = C_GT.(λ, λ .^ (-0.5), ρs, VL, GTfit[1], GTfit[2], fop, τ, n, βGT)
    ...
    CCop = C_C.(λ, λ .^ (-0.5), ρs, VL, Cfit[1], Cfit[2], fop, τ, n, βC)
