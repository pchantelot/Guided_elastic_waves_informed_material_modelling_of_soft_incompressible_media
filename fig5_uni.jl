...
with_theme(My_theme, palette=(color=ColorSchemes.Set1_3, marker=[:circle])) do
    ...
    # add predictions
    CMRip = C_MR.(λ, λ .^ (-0.5), ρs, VL, MRfit[1], MRfit[2], fip, τ, n, βMR)
    ...
    CMRSHip = C_MRSH.(λ, λ .^ (-0.5), ρs, VL, MRSHfit[1], MRSHfit[2], MRSHfit[3], MRSHfit[4], fip, τ, n, βMRSH)
    ...
    CCSHip = C_CSH.(λ, λ .^ (-0.5), ρs, VL, CSHfit[1], CSHfit[2], CSHfit[3], CSHfit[4], fip, τ, n, βCSH)
    ...
    CMRop = C_MR.(λ, λ .^ (-0.5), ρs, VL, MRfit[1], MRfit[2], fop, τ, n, βMR)
    ...
    CMRSHop = C_MRSH.(λ, λ .^ (-0.5), ρs, VL, MRSHfit[1], MRSHfit[2], MRSHfit[3], MRSHfit[4], fop, τ, n, βMRSH)
    ...
    CCSHop = C_CSH.(λ, λ .^ (-0.5), ρs, VL, CSHfit[1], CSHfit[2], CSHfit[3], CSHfit[4], fop, τ, n, βCSH)
    ...
    rowgap!(fig.layout, 0)
    display(fig)
    save(joinpath(@__DIR__, "figure5_uniaxial.pdf"), fig; pt_per_unit=1.0)
